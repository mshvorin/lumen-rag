...
export async function GET(request:Request){try{
  const user=await owner(request);const result=await database().prepare("SELECT id,name,bytes,chunks,words,created_at FROM documents WHERE owner=? ORDER BY created_at DESC").bind(user).all();
...
export async function POST(request:Request){let key:string|undefined;try{
  ...
  const bytes=await file.arrayBuffer();const hash=Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",bytes))).map(b=>b.toString(16).padStart(2,"0")).join("");
  ...
  if(!duplicate){key=`documents/${id}`;await bucket().put(key,bytes,{httpMetadata:{contentType:"application/octet-stream"}});}
  const words=pages.reduce((n:number,p:any)=>n+(p.text.trim()?p.text.trim().split(/\s+/).length:0),0);
  const indexedAt=new Date().toISOString();
  const insert=duplicate?db.prepare("UPDATE documents SET chunks=?,words=?,created_at=? WHERE id=? AND owner=? AND (SELECT COALESCE(SUM(chunks),0) FROM documents WHERE owner=?) - chunks + ? <= 3000").bind(passages.length,words,indexedAt,id,user,user,passages.length):db.prepare("INSERT INTO documents (id,owner,name,hash,bytes,chunks,words,created_at) SELECT ?,?,?,?,?,?,?,? WHERE (SELECT COALESCE(SUM(chunks),0) FROM documents WHERE owner=?) + ? <= 3000").bind(id,user,file.name.slice(0,255),hash,file.size,passages.length,words,indexedAt,user,passages.length);
