...
type Row={source_url:string|null;id:number;name:string;content:string;page:number|null;position:number;embedding:string};
export async function POST(request:Request){try{
  ...
  while(true){
    const {results}=await db.prepare("SELECT c.id,d.name,d.source_url,c.content,c.page,c.position,c.embedding FROM chunks c JOIN documents d ON d.id=c.document_id WHERE d.owner=? AND c.id>? ORDER BY c.id LIMIT 150").bind(user,last).all<Row>();
