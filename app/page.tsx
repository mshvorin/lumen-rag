...
import { Globe, ArrowUp, ArrowUpRight, Check, ChevronRight, FileText, FolderOpen, Layers3, LoaderCircle, Plus, ScanText, Search, ShieldCheck, SlidersHorizontal, Sparkles, Trash2, Upload, X } from "lucide-react";
import { WebsiteImporter } from "@/components/website-importer";
import { Slider } from "@/components/ui/slider";
import { Dialog, DialogContent, DialogDescription, DialogTitle } from "@/components/ui/dialog";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogTitle } from "@/components/ui/alert-dialog";

type Doc = { source_url?:string|null; id: string; name: string; bytes: number; chunks: number; words: number };
type Source = { source_url?:string|null; name: string; content: string; score: number; position: number; page: number | null };
...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    try{Promise.resolve(context.registerTool({name:"ask_documents",title:"Ask your documents",description:"Search the current document collection and generate a cited answer using the current similarity threshold. Calls OpenAI and displays the result.",inputSchema:{type:"object",properties:{question:{type:"string",minLength:1,maxLength:2000}},required:["question"],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:true},async execute(input:unknown){
      ...
      return {answer:result.answer,sources:result.sources.map((s:Source)=>({name:s.name,url:s.source_url,page:s.page,score:s.score}))};
  ...
  return <div className="app-shell">
    ...
    <main><div className="page-heading"><div><div className="eyebrow">YOUR KNOWLEDGE, CONNECTED</div><h1>A little clarity. From every file.</h1><p>Bring your documents together. Find the answers inside.</p></div><button className="secondary" onClick={()=>setSettings(true)}><SlidersHorizontal size={16}/>Retrieval settings</button></div>
      ...
      <div className="workspace"><section className="collection panel" aria-labelledby="collection-title"><div className="section-heading"><div className="section-title"><FolderOpen size={19}/><h2 id="collection-title">Your collection</h2><span className="count">{docs.length}</span></div><button className="icon-button" aria-label="Add documents" disabled={!!busy||asking} onClick={()=>input.current?.click()}><Plus size={19}/></button></div>
        ...
        <WebsiteImporter disabled={!!busy||asking} onBusy={setBusy} onComplete={refresh}/>
        <div className="file-list" aria-live="polite">{loading?<div className="collection-empty"><LoaderCircle className="spin" size={22}/><p>Loading your collection…</p></div>:docs.length?docs.map(doc=><div className="file-row" key={doc.id}><span className={`file-icon ${doc.name.toLowerCase().endsWith(".pdf")?"pdf":""}`}>{doc.source_url?<Globe size={20}/>:<FileText size={20}/>}</span><div className="file-info"><strong title={doc.name}>{doc.name}</strong><small>{doc.source_url&&<>{new URL(doc.source_url).hostname} · </>}{doc.chunks} passages · {doc.words.toLocaleString()} words</small><span className="ready"><Check size={11}/>Ready to search</span></div><button className="icon-button delete" aria-label={`Delete ${doc.name}`} onClick={()=>setRemove(doc)} disabled={!!busy||asking}><Trash2 size={15}/></button></div>):<div className="collection-empty"><div className="empty-file-icon"><FileText size={23}/></div><strong>Your knowledge starts here</strong><p>Add a document to make its contents<br/>searchable and ready for questions.</p></div>}</div>
        <div className="collection-footer"><span><Layers3 size={14}/>{totalChunks.toLocaleString()} indexed passages</span><span>{docs.length} {docs.length===1?"source":"sources"}</span></div></section>
    ...
    <Dialog open={!!selected} onOpenChange={open=>{if(!open)setSelected(null);}}><DialogContent className="source-dialog"><DialogTitle>{selected?.name}</DialogTitle><DialogDescription>{selected?.page?`Page ${selected.page}`:`Passage ${(selected?.position||0)+1}`} · cosine similarity {selected?.score.toFixed(3)}</DialogDescription>{selected?.source_url&&<a className="source-link" href={selected.source_url} target="_blank" rel="noopener noreferrer">Open original page <ArrowUpRight size={15}/></a>}<div className="source-content">{selected?.content}</div></DialogContent></Dialog>
